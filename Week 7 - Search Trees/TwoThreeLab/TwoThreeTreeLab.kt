...
class TwoThreeTree {
    ...
    */
    private fun insertHelp(subroot: Node?, key: Int) : Node? {
        ...
        if (subroot == null) {
            return newNode
        } else if (subroot.left == null && subroot.center == null && subroot.right == null) {
            return add(subroot, newNode)
        } else if (key < subroot.lval) {
            return insertLeft(subroot, key)
        } else if (subroot.rval == null || key < subroot.rval!!) {
            return insertCenter(subroot, key)
        } else {
            return insertRight(subroot, key)
        }
        return null
    }

    private fun insertLeft(subroot: Node?, key : Int) : Node?{
        val leftNode = insertHelp(subroot?.left, key)
        if (leftNode == subroot?.left) {
            return subroot
        } else {
            return add(subroot!!, leftNode!!)
        }
        return null
    }

    private fun insertCenter(subroot: Node?, key : Int) : Node?{
        val centerNode = insertHelp(subroot?.center, key)
        if (centerNode == subroot?.center) {
            return subroot
        } else {
            return add(subroot!!, centerNode!!)
        }
        return null
    }

    private fun insertRight(subroot: Node?, key : Int) : Node?{
        val rightNode = insertHelp(subroot?.right, key)
        if (rightNode == subroot?.right) {
            return subroot
        } else {
            return add(subroot!!, rightNode!!)
        }
        return null
    }
