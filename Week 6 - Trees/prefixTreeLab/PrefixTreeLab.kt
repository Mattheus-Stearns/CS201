...
fun deleteKey(root: TrieNode, word: String): Boolean {
    ...
    var curr: TrieNode? = root
    val nodesToUpdate = mutableListOf<Pair<TrieNode, Int>>()
    for (letter in word) {
        val index = letter - 'a' //this will get you the index, cool huh??
        if (curr?.childNode?.get(index) == null) {
            return false
        }
        nodesToUpdate.add(Pair(curr!!, index))
        curr = curr.childNode[index]
    }

    val lastNode = curr!!

    if (hasChildren(lastNode) || lastNode.wordCount > 1) {
        lastNode.wordCount--
        return true
    }
    ...
    for (i in nodesToUpdate.size - 1 downTo 0) {
        val (parent, index) = nodesToUpdate[i]
        val node = parent.childNode[index]!!
        
        if (!hasChildren(node) && node.wordCount == 0) {
...
private fun hasChildren(node: TrieNode): Boolean {
